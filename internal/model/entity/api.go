...
type API struct {
	...
	SyncState  string     `json:"sync_state" gorm:"type:varchar(20);not null;default:'registered';index:idx_api_sync_state;comment:路由同步状态(registered/missing/archived)"`
	LastSeenAt *time.Time `json:"last_seen_at,omitempty" gorm:"index;comment:最近一次路由扫描命中时间"`
}
