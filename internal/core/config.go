...
func InitConfig(path string) {
	...
	viper.SetDefault("task.oj_task_dispatch_enabled", true)
	viper.SetDefault("task.oj_task_dispatch_interval_seconds", 10)
	viper.SetDefault("task.oj_task_dispatch_batch_size", 10)
	viper.SetDefault("task.oj_task_dispatch_worker_count", 1)
	viper.SetDefault("task.oj_task_snapshot_insert_batch_size", 500)
	viper.SetDefault("task.oj_task_execution_lock_ttl_seconds", 60)
	...
	viper.SetDefault("sse.heartbeat_interval_seconds", 20)
	viper.SetDefault("sse.write_timeout_seconds", 10)
	viper.SetDefault("sse.queue_capacity", 64)
	viper.SetDefault("sse.max_connections_per_subject", 3)
	viper.SetDefault("sse.replay_limit", 100)
	viper.SetDefault("sse.allowed_origins", []string{
		"http://localhost:3000",
		"http://127.0.0.1:3000",
		"http://192.168.20.14:3000",
	})
	viper.SetDefault("sse.drain_timeout_seconds", 15)
	viper.SetDefault("sse.pubsub_channel_prefix", "sse")
	viper.SetDefault("sse.replay_stream_prefix", "sse:replay")
	viper.SetDefault("sse.ai_runtime_mode", "eino")
	viper.SetDefault("ai.provider", "qwen")
	viper.SetDefault("ai.base_url", "https://dashscope.aliyuncs.com/compatible-mode/v1")
	viper.SetDefault("ai.model", "qwen-plus")
	viper.SetDefault("ai.system_prompt", "你是 personal_assistant 项目的 AI 助手。当前阶段只提供基础流式对话，不调用工具，不请求人工确认。请直接、准确地回答用户问题。")
	viper.SetDefault("ai.temperature", 0.2)
	viper.SetDefault("ai.max_completion_tokens", 1200)
	viper.SetDefault("ai.memory.enabled", false)
	viper.SetDefault("ai.memory.recall_top_k", 6)
	viper.SetDefault("ai.memory.recall_max_chars", 2000)
	viper.SetDefault("ai.memory.recall_min_score", 0.2)
	viper.SetDefault("ai.memory.rag_max_chars", 2000)
	viper.SetDefault("ai.memory.recent_raw_turns", 8)
	viper.SetDefault("ai.memory.recent_raw_token_budget", 3000)
	viper.SetDefault("ai.memory.compress_threshold_tokens", 6000)
	viper.SetDefault("ai.memory.summary_refresh_every_turns", 10)
	viper.SetDefault("ai.memory.writeback_async", true)
	viper.SetDefault("ai.memory.enable_entity_memory", true)
	viper.SetDefault("ai.memory.enable_long_term_memory", true)
	viper.SetDefault("ai.memory.enable_org_memory", true)
	viper.SetDefault("ai.memory.enable_ops_memory", true)
	viper.SetDefault("ai.memory.min_importance", 0.65)
	viper.SetDefault("ai.memory.extractor_mode", "rule")
	viper.SetDefault("ai.memory.extract_timeout_seconds", 20)
	viper.SetDefault("ai.memory.extract_max_chars", 6000)
	viper.SetDefault("ai.memory.tool_output_token_budget", 800)
	viper.SetDefault("ai.memory.embed_model", "qwen3-vl-embedding")
	viper.SetDefault("ai.memory.embed_endpoint", "https://dashscope.aliyuncs.com/api/v1/services/embeddings/multimodal-embedding/multimodal-embedding")
	viper.SetDefault("ai.memory.embed_dimension", 1024)
	viper.SetDefault("ai.memory.chunk_max_chars", 1200)
	viper.SetDefault("ai.memory.chunk_overlap_chars", 150)
	viper.SetDefault("ai.memory.index_batch_size", 20)
	viper.SetDefault("ai.memory.index_timeout_seconds", 30)
	viper.SetDefault("qdrant.enabled", true)
	viper.SetDefault("qdrant.endpoint", "")
	viper.SetDefault("qdrant.grpc_host", "")
	viper.SetDefault("qdrant.grpc_port", 6334)
	viper.SetDefault("qdrant.api_key", "")
	viper.SetDefault("qdrant.collection_name", "ai_knowledge_chunks")
	viper.SetDefault("qdrant.knowledge_collection_name", "ai_knowledge_chunks")
	viper.SetDefault("qdrant.memory_collection_name", "ai_memory_chunks")
	viper.SetDefault("qdrant.vector_size", 1024)
	viper.SetDefault("qdrant.distance", "cosine")
	viper.SetDefault("qdrant.init_collection", true)
	viper.SetDefault("qdrant.timeout_seconds", 10)
	...
	_ = viper.BindEnv("task.oj_task_dispatch_enabled", "TASK_OJ_TASK_DISPATCH_ENABLED")
	_ = viper.BindEnv("task.oj_task_dispatch_interval_seconds", "TASK_OJ_TASK_DISPATCH_INTERVAL_SECONDS")
	_ = viper.BindEnv("task.oj_task_dispatch_batch_size", "TASK_OJ_TASK_DISPATCH_BATCH_SIZE")
	_ = viper.BindEnv("task.oj_task_dispatch_worker_count", "TASK_OJ_TASK_DISPATCH_WORKER_COUNT")
	_ = viper.BindEnv("task.oj_task_snapshot_insert_batch_size", "TASK_OJ_TASK_SNAPSHOT_INSERT_BATCH_SIZE")
	_ = viper.BindEnv("task.oj_task_execution_lock_ttl_seconds", "TASK_OJ_TASK_EXECUTION_LOCK_TTL_SECONDS")
	...
	_ = viper.BindEnv("messaging.oj_question_upsert_topic", "MESSAGING_OJ_QUESTION_UPSERT_TOPIC")
	_ = viper.BindEnv("messaging.oj_question_upsert_group", "MESSAGING_OJ_QUESTION_UPSERT_GROUP")
	_ = viper.BindEnv("messaging.oj_question_upsert_consumer", "MESSAGING_OJ_QUESTION_UPSERT_CONSUMER")
	...
	_ = viper.BindEnv("sse.heartbeat_interval_seconds", "SSE_HEARTBEAT_INTERVAL_SECONDS")
	_ = viper.BindEnv("sse.write_timeout_seconds", "SSE_WRITE_TIMEOUT_SECONDS")
	_ = viper.BindEnv("sse.queue_capacity", "SSE_QUEUE_CAPACITY")
	_ = viper.BindEnv("sse.max_connections_per_subject", "SSE_MAX_CONNECTIONS_PER_SUBJECT")
	_ = viper.BindEnv("sse.replay_limit", "SSE_REPLAY_LIMIT")
	_ = viper.BindEnv("sse.allowed_origins", "SSE_ALLOWED_ORIGINS")
	_ = viper.BindEnv("sse.drain_timeout_seconds", "SSE_DRAIN_TIMEOUT_SECONDS")
	_ = viper.BindEnv("sse.pubsub_channel_prefix", "SSE_PUBSUB_CHANNEL_PREFIX")
	_ = viper.BindEnv("sse.replay_stream_prefix", "SSE_REPLAY_STREAM_PREFIX")
	_ = viper.BindEnv("sse.ai_runtime_mode", "SSE_AI_RUNTIME_MODE")
	_ = viper.BindEnv("ai.provider", "AI_PROVIDER")
	_ = viper.BindEnv("ai.api_key", "AI_API_KEY")
	_ = viper.BindEnv("ai.base_url", "AI_BASE_URL")
	_ = viper.BindEnv("ai.model", "AI_MODEL")
	_ = viper.BindEnv("ai.by_azure", "AI_BY_AZURE")
	_ = viper.BindEnv("ai.api_version", "AI_API_VERSION")
	_ = viper.BindEnv("ai.system_prompt", "AI_SYSTEM_PROMPT")
	_ = viper.BindEnv("ai.temperature", "AI_TEMPERATURE")
	_ = viper.BindEnv("ai.max_completion_tokens", "AI_MAX_COMPLETION_TOKENS")
	_ = viper.BindEnv("ai.memory.enabled", "AI_MEMORY_ENABLED")
	_ = viper.BindEnv("ai.memory.recall_top_k", "AI_MEMORY_RECALL_TOP_K")
	_ = viper.BindEnv("ai.memory.recall_max_chars", "AI_MEMORY_RECALL_MAX_CHARS")
	_ = viper.BindEnv("ai.memory.recall_min_score", "AI_MEMORY_RECALL_MIN_SCORE")
	_ = viper.BindEnv("ai.memory.rag_max_chars", "AI_MEMORY_RAG_MAX_CHARS")
	_ = viper.BindEnv("ai.memory.recent_raw_turns", "AI_MEMORY_RECENT_RAW_TURNS")
	_ = viper.BindEnv("ai.memory.recent_raw_token_budget", "AI_MEMORY_RECENT_RAW_TOKEN_BUDGET")
	_ = viper.BindEnv("ai.memory.compress_threshold_tokens", "AI_MEMORY_COMPRESS_THRESHOLD_TOKENS")
	_ = viper.BindEnv("ai.memory.summary_refresh_every_turns", "AI_MEMORY_SUMMARY_REFRESH_EVERY_TURNS")
	_ = viper.BindEnv("ai.memory.writeback_async", "AI_MEMORY_WRITEBACK_ASYNC")
	_ = viper.BindEnv("ai.memory.enable_entity_memory", "AI_MEMORY_ENABLE_ENTITY_MEMORY")
	_ = viper.BindEnv("ai.memory.enable_long_term_memory", "AI_MEMORY_ENABLE_LONG_TERM_MEMORY")
	_ = viper.BindEnv("ai.memory.enable_org_memory", "AI_MEMORY_ENABLE_ORG_MEMORY")
	_ = viper.BindEnv("ai.memory.enable_ops_memory", "AI_MEMORY_ENABLE_OPS_MEMORY")
	_ = viper.BindEnv("ai.memory.min_importance", "AI_MEMORY_MIN_IMPORTANCE")
	_ = viper.BindEnv("ai.memory.extractor_mode", "AI_MEMORY_EXTRACTOR_MODE")
	_ = viper.BindEnv("ai.memory.extract_timeout_seconds", "AI_MEMORY_EXTRACT_TIMEOUT_SECONDS")
	_ = viper.BindEnv("ai.memory.extract_max_chars", "AI_MEMORY_EXTRACT_MAX_CHARS")
	_ = viper.BindEnv("ai.memory.tool_output_token_budget", "AI_MEMORY_TOOL_OUTPUT_TOKEN_BUDGET")
	_ = viper.BindEnv("ai.memory.embed_model", "AI_MEMORY_EMBED_MODEL")
	_ = viper.BindEnv("ai.memory.embed_endpoint", "AI_MEMORY_EMBED_ENDPOINT")
	_ = viper.BindEnv("ai.memory.embed_dimension", "AI_MEMORY_EMBED_DIMENSION")
	_ = viper.BindEnv("ai.memory.chunk_max_chars", "AI_MEMORY_CHUNK_MAX_CHARS")
	_ = viper.BindEnv("ai.memory.chunk_overlap_chars", "AI_MEMORY_CHUNK_OVERLAP_CHARS")
	_ = viper.BindEnv("ai.memory.index_batch_size", "AI_MEMORY_INDEX_BATCH_SIZE")
	_ = viper.BindEnv("ai.memory.index_timeout_seconds", "AI_MEMORY_INDEX_TIMEOUT_SECONDS")
	_ = viper.BindEnv("qdrant.enabled", "QDRANT_ENABLED")
	_ = viper.BindEnv("qdrant.endpoint", "QDRANT_ENDPOINT")
	_ = viper.BindEnv("qdrant.grpc_host", "QDRANT_GRPC_HOST")
	_ = viper.BindEnv("qdrant.grpc_port", "QDRANT_GRPC_PORT")
	_ = viper.BindEnv("qdrant.api_key", "QDRANT_API_KEY")
	_ = viper.BindEnv("qdrant.collection_name", "QDRANT_COLLECTION_NAME")
	_ = viper.BindEnv("qdrant.knowledge_collection_name", "QDRANT_KNOWLEDGE_COLLECTION_NAME")
	_ = viper.BindEnv("qdrant.memory_collection_name", "QDRANT_MEMORY_COLLECTION_NAME")
	_ = viper.BindEnv("qdrant.vector_size", "QDRANT_VECTOR_SIZE")
	_ = viper.BindEnv("qdrant.distance", "QDRANT_DISTANCE")
	_ = viper.BindEnv("qdrant.init_collection", "QDRANT_INIT_COLLECTION")
	_ = viper.BindEnv("qdrant.timeout_seconds", "QDRANT_TIMEOUT_SECONDS")
