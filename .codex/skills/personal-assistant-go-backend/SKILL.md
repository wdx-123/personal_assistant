...
- 在 `internal/model/config` 增加设施配置结构（开关、endpoint、timeout、采样率、batch 等）。
- 通过 `configs` 驱动，运行时仅从 `global.Config` 读取。
- 仅保留必要零值兜底，禁止硬编码运行参数。
- `pkg/*` 若需可复用配置，定义模块内 `Options` 结构体；由 `core`/`init` 映射后注入。
- 禁止在 `pkg/*` 直接读取 `viper` 或 `global.Config`。

### 第 2 步：Core

- 在 `internal/core` 实现设施初始化、连接创建与生命周期管理。
- 初始化失败返回原始 `error`，交给上层编排决定是否 fail-fast。
- 避免在 core 外部重复创建同类客户端实例。

### 第 3 步：Init

- 在 `internal/init/init.go` 按依赖顺序编排初始化。
- 仅编排，不承载具体初始化细节。
- 确保同一设施不会重复启动。
...
- 通用调用能力优先沉淀到 `pkg/*`。
- HTTP 入口接入放在 `internal/middleware` 或 `internal/router/system`。
- `internal/router/router.go` 只做挂载，不写设施业务细节。

### 第 5 步：业务层使用

- Controller 不直接依赖第三方设施 SDK。
- Service 不直接 `new` 第三方客户端；统一通过 `pkg/*` 或已初始化实例调用。
- Repository 继续只负责数据访问，不承担设施编排。
- Redis/MySQL 等基础资源的 CRUD/JOIN 继续由 Repository 承担，禁止下沉到“通用基础设施层”。

### 第 6 步：验证

- 验证开关生效、降级路径、日志可观测性。
- 至少执行最小必要的 build/test 或启动自检。
...
- 硬编码可变运行参数。
- 在 Controller/Service 直接 `new` 第三方基础设施客户端。
- 在 `internal/init/init.go` 写具体设施初始化细节。
- 重复初始化或重复启动同一基础设施组件。
